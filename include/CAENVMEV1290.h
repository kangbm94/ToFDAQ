...
Output buffer structure and memory map of V1290
-----------------------------------------------------------------------------
*/
...
// V1290 Output Buffer bit field
// output buffer, ro, D16
typedef struct _V1290_OUTBUF_REG{
	union {
		struct {
			uint32_t data         : 12;
			uint32_t overFlow     : 1;
...
class CAENVMEV1290:public CAENVMEModule{
	public:
		CAENVMEV1290(){name+="v1290";};
		~CAENVMEV1290(){};
		void SetNType(bool type_){
			type=type_;
		}
		//TDC Settings//
...
//		void SetDeadTime( int width);
		...
		uint32_t AssignBlockedData(uint32_t* Block, int32_t* data);		
};
