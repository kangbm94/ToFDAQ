...
CAENVME_API CvInit2(void* arg, int32_t *handle)
{
 // CAENVME_API ret = CAENVME_Init(cvV1718, 0, bdNum, handle);
  ...
  CAENVME_API ret = CAENVME_Init2(cvUSB_V4718, arg,0, handle);
//  CAENVME_API ret = CAENVME_Init2(cvETH_V4718, arg,0, handle);
  if (ret != cvSuccess)
    PrintErrInfo("CvInit", "CAENVME_Init", "Failed to open V4718.", ret);
  else
  {
    ret = CAENVME_SystemReset(*handle);
    if (ret != cvSuccess)
      PrintErrInfo("CvReset", "CAENVME_SystemReset", "Failed to reset V4718.", ret);
  	else
			printf("Initialized\n");
	}
  return ret;
...
int CvReadBLT(int32_t handle, uint32_t addr, int size, uint32_t* buff){
  int count;
	CAENVME_API ret = CAENVME_BLTReadCycle(handle, addr, (unsigned char*)buff, size, cvA32_U_BLT, cvD32, &count);
  if (ret != cvSuccess)
  {
    char msg[256];
    sprintf(msg, "Failed to read the register at address 0x%08X.", (uint32_t)(addr));
